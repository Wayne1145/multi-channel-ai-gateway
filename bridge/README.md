...
- 文本与媒体收发：图片、文件、语音、视频；
- 入站图片/语音/视频仍只转发安全元数据；PDF/TXT/Markdown/HTML/DOCX 在 Bridge 内从官方 CDN 限额下载并 AES 解密，只把解密后的短生命周期字节经内部 Bearer API 交给网关提取文本，CDN 参数与密钥不跨进程、不落日志；
...
- `GET /instances/{id}/status` 以 Bridge 内存状态为实时来源，仅返回白名单公开字段；二维码过期会自动换码，普通网络故障按 1/2/5/10/30 秒退避重连。
- 网关每 30 秒对账状态并记录最后检查、最后在线、错误类型与故障/恢复告警；可运行 `python -m wecom_ai_gateway.soak --duration-hours 72` 做脱敏长稳采样。
