...
- TOTP 多因素认证：普通用户与管理员均可在「我的设置」启用；秘钥 Fernet 加密，恢复码仅存 SHA-256 摘要并一次性消费；密码通过后只签发 5 分钟单用途挑战，验证成功才创建正式会话；TOTP 计数器防重放，错误验证码计入登录失败锁定；启用时撤销其他会话，停用/管理员重置时撤销全部会话；管理员启用后禁用旧 `X-Admin-Token` 单因素旁路；
...
- 网关单元、API、安全、回调、隔离、Outbox、渠道、媒体、账号中心、TOTP MFA、运行时设置、覆盖框架、自助中心、模型路由、受限工具及迁移测试（本地全量 231 项）；
- Bridge 独立测试 42 项，公开镜像构建与非 root 导入验证通过；
- Ruff 与 Python `compileall` 静态检查；`git diff --check`；
- 生产 PostgreSQL 迁移链已升级至 0013（TOTP MFA 凭据与短时挑战），API/Worker/ClawBot Bridge healthy；临时普通账号完成启用、挑战登录、恢复码一次性消费、管理员重置与清理的真实闭环；
