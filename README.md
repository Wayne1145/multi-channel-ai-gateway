...
- 长期记忆加密存储（新写入数据入库即密文）
- 平台设置中心：11 组 65 项运行时可调参数（模型/受限工具/配额/上限/媒体/任务/保留策略/告警等），带类型与上下限校验、审计日志，保存即生效；支持渠道/用户级覆盖
- 用户自助中心：网页管理角色卡（含 PNG 导入）、预设、长期记忆、自带密钥（BYOK）、账号安全（改密码/会话管理/TOTP 多因素认证）、用量曲线
- 受限只读工具执行（默认关闭）：仅接受 OpenAI 标准结构化 tool_calls，内置当前时间与 Open-Meteo 天气查询，带白名单、参数校验、超时、调用上限与脱敏审计
- 长回复自动分片发送（默认 1500 字符/条，可配置）
- 维护模式与系统公告；数据保留清理（默认关闭）；登录 IP 限流；邮件告警（默认关闭）
- 每用户独立身份、会话、模型、人设和参数
- 私有长期记忆（默认关闭）
- 每日 Token 配额
...
已有微信用户发送 `/account` 后会收到一个 15 分钟有效、仅限一次的激活链接。
打开链接后由用户自行设置用户名和密码，系统会把后台账号直接建立在原微信用户上，
...
- 普通用户与管理员均支持 TOTP MFA：秘钥加密存储，登录使用短时单用途挑战，恢复码只存哈希且一次性消费；启用、停用和管理员重置都会撤销不应继续有效的会话。
- 工具执行默认关闭且仅限静态只读白名单；不解析文本函数调用，不支持 Shell、文件、任意 URL、内网访问或设备控制。
- `.env`、数据库、日志和上传文件不会进入 Git。
- 管理 API 默认兼容 `X-Admin-Token`；管理员启用 MFA 后，旧的单因素 Header 入口会被关闭，必须使用完成 MFA 的 Bearer 会话。
- 长期记忆默认关闭，并只能由当前用户管理。
